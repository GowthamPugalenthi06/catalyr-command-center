...
const generateDayTasks = (agents, date, isCompleted = false) => {
    ...
    const addTask = (agent, title, desc, durationHours = 2) => {
        ...
        startTime.setHours(baseHour + (Math.floor(Math.random() * 8)), 0, 0, 0);
...
const run = async () => {
    ...
    try {
        console.log("--- Reset to Today Only ---");
        ...
        // 3. Generate Today (PENDING - Fresh for today)
        const today = new Date();
        console.log(`Generating tasks for Today (${today.toLocaleDateString()}) ONLY...`);
        const todayTasks = generateDayTasks(dbAgents, today, false); // false = PENDING
        await Task.insertMany(todayTasks);

        console.log(`Success! Created ${todayTasks.length} tasks for Today.`);
        process.exit(0);
